...
export const STARTUPS_QUERY = defineQuery(`*[_type== "startup" && defined(slug.current) && !defined($search) || title match $search || category match $search || author -> match $search] | order(_createdAt desc) {
    _id,
    title,
    slug,
    author -> {
